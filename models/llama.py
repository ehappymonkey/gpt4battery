from transformers import LlamaModel, LlamaConfig, BertModel, BertTokenizer
...
class LlamaSOH(nn.Module):
    def __init__(self, input_dim, llama_model_path, PPA=False, soft_prompt_len=10):
        ...
        # self.llama = LlamaModel.from_pretrained('huggyllama/llama-7b',
        #             trust_remote_code=True,
        #             local_files_only=False,
        #             config=LlamaConfig.from_pretrained('huggyllama/llama-7b'),)
        self.llama = LlamaModel.from_pretrained(llama_model_path)
        
        self.output_linear = nn.Linear(4096, input_dim)

        # Freeze LLama parameters except for LayerNorm and positional encodings
        ...
        if self.PPA:
            ...
            self.soft_prompt = nn.Parameter(torch.zeros(self.soft_prompt_len, 4096))
    ...
    def forward(self, src):
        ...
        if self.PPA:
            ...
            # Use LLama model
            llama_outputs = self.llama(inputs_embeds=src_with_prompt)
            ...
            llama_outputs = self.llama(inputs_embeds=src)
            hidden_states = llama_outputs.last_hidden_state
